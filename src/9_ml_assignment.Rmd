...
[1]	train-rmse:1.758348	val-rmse:1.873447 
...
[24]	train-rmse:0.703952	val-rmse:0.768481
...
## Format it the way xgboost likes
dtrain_fd_bm5J <- xgb.DMatrix(data = as.matrix(train_fiveDay[,band_met5_feats]), 
                      label = train_fiveDay[,target])
dtest_fd_bm5J <- xgb.DMatrix(data = as.matrix(test_fiveDay[,band_met5_feats]), 
                     label = test_fiveDay[,target])
```
...
#plot the results
ggplot(preds_fd_bm5_play, aes(x = secchi, y = predicted_fd_bm5J)) + 
  geom_point() +
  geom_abline(color = 'grey', lty = 2) + 
  coord_cartesian(xlim = c(0, 6.5),
                  ylim = c(0,6.5)) +
